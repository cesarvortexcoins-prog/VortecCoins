...
async function handleAuthSubmit(e) {
    ...
    if (currentAuthMode === 'login') {
        ...
        if (error) {
            ...
            errorDiv.textContent = upsertError ? "¡Registro exitoso! Confirma tu correo para iniciar sesión." : "¡Registro exitoso! Iniciando sesión...";
        }
    }
}
...
async function renderAdmin() {
    ...
    appContainer.innerHTML = `
        ...
        <div class="admin-tabs" style="-webkit-overflow-scrolling: touch;">
            <div class="admin-tab" id="tab-btn-orders" onclick="switchAdminTab('orders')">📦 Pedidos</div>
            <div class="admin-tab" id="tab-btn-products" onclick="switchAdminTab('products')">🛍️ Productos</div>
            <div class="admin-tab" id="tab-btn-design" onclick="switchAdminTab('design')">🎨 Diseño e Imágenes</div>
            <div class="admin-tab" id="tab-btn-deposits" onclick="switchAdminTab('deposits')">💰 Depósitos de Saldo</div>
            <div class="admin-tab" id="tab-btn-clients" onclick="switchAdminTab('clients')">👥 Clientes y Rangos</div>
            <div class="admin-tab" id="tab-btn-promotions" onclick="switchAdminTab('promotions')">🎠 Promociones</div>
            <div class="admin-tab" id="tab-btn-inventory" onclick="switchAdminTab('inventory')">📦 Inventario Auto-Entrega</div>
            <div class="admin-tab" id="tab-btn-alerts" onclick="switchAdminTab('alerts')">📢 Alertas Globales</div>
            <div class="admin-tab" id="tab-btn-settings" onclick="switchAdminTab('settings')">⚙️ Configuraciones</div>
        </div>
        
        <!-- ADMIN MODAL OVERLAY -->
        <div class="modal-overlay" id="admin-modal-overlay">
            <div class="checkout-modal" style="width: 95%; max-width: 1200px; max-height: 90vh; display: flex; flex-direction: column; padding: 0;">
                <div class="modal-header" style="padding: 15px 20px; border-bottom: 1px solid rgba(255,255,255,0.1); background: var(--bg-surface); border-radius: 12px 12px 0 0;">
                    <h3 id="admin-modal-title" style="font-family: var(--font-heading); color: var(--gold); margin: 0;">Panel</h3>
                    <button class="modal-close" onclick="closeAdminModal()" style="margin:0;">&times;</button>
                </div>
                <div id="admin-content-area" style="overflow-y: auto; overflow-x: hidden; flex-grow: 1; padding: 20px; background: var(--bg-dark); border-radius: 0 0 12px 12px;">
                </div>
            </div>
        </div>
    `;
...
window.switchAdminTab = function(tab) {
    try {
        ...
        const tabEl = document.getElementById('tab-btn-' + tab);
        document.getElementById('admin-modal-title').innerHTML = tabEl.innerHTML;
                            ...
                            <td data-label="Acciones">
                                ...
                                ${order.profiles?.whatsapp ? `
                                    ${order.status !== 'rejected' ? `
                                        <a href="https://wa.me/${order.profiles.whatsapp.replace(/\\D/g, '')}?text=${encodeURIComponent(`¡Hola! Tu pedido de ${order.products?.name} ha sido realizado con éxito. ¡Gracias por preferir Vortex Coins!`)}" target="_blank" class="btn btn-outline" style="padding: 3px 5px; font-size: 10px; display: block; text-align: center; margin-bottom: 5px;">💬 Notificar Éxito</a>
                                    ` : `
                                        <a href="https://wa.me/${order.profiles.whatsapp.replace(/\\D/g, '')}?text=${encodeURIComponent(`¡Hola! Tu pedido de ${order.products?.name} ha sido rechazado.${order.order_data?.['Motivo de Rechazo'] ? ' Motivo: ' + order.order_data['Motivo de Rechazo'] : ''} Por favor comunícate con nosotros para más información.`)}" target="_blank" class="btn btn-outline" style="padding: 3px 5px; font-size: 10px; display: block; text-align: center; color: #ef4444; border-color: #ef4444; margin-bottom: 5px;">💬 Notificar Rechazo</a>
                                    `}
                                ` : ''}
                            </td>
                        </tr>
        ...
        for (const sub in grouped[cat]) {
            ...
            grouped[cat][sub].forEach(p => {
                html += `
                    <tr id="row-prod-${p.id}">
                        <td>${p.name}</td>
                        <td><input type="text" id="subcat-${p.id}" value="${p.sub_category || ''}" placeholder="Ej: Pases" style="width:100%; min-width:100px; background:var(--bg-dark); color:white; border:1px solid #333; padding:5px; border-radius:4px;"></td>
                        <td>
                        ...
                        <td><input type="number" id="prov-${p.id}" value="${p.price_usdt_proveedor}" step="0.01" style="width:60px; background:var(--bg-dark); color:white; border:1px solid #333; padding:5px; border-radius:4px;"></td>
                        <td><input type="number" id="cli-${p.id}" value="${p.price_usdt_cliente}" step="0.01" style="width:60px; background:var(--bg-dark); color:white; border:1px solid #333; padding:5px; border-radius:4px;"></td>
                        <td><input type="number" id="esp-${p.id}" value="${p.price_usdt_especial}" step="0.01" style="width:60px; background:var(--bg-dark); color:white; border:1px solid #333; padding:5px; border-radius:4px;"></td>
                        <td><input type="number" id="rev-${p.id}" value="${p.price_usdt_revendedor}" step="0.01" style="width:60px; background:var(--bg-dark); color:white; border:1px solid #333; padding:5px; border-radius:4px;"></td>
                        <td>
